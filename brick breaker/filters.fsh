...
vec4 invert() {
	...
	vec4 main_color = vec4(color.g, color.r, color.g, color.a);
	return main_color;
}
...
vec4 retro_bit() {
	...
	vec2 tc = vec2(dx * floor(uv.x/dx), dy * floor(uv.y/dy));
	vec4 color = texture2D(u_texture, tc);
	vec4 main_color = vec4(color.r, color.g, color.b, color.a);
	return main_color;
}
